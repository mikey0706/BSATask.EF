...
public class DataProcessingService : IDataProcessingService
{
    ...
    public DataProcessingService(IDataProvider provider, IMapper mapper)
    {
    ...
    {
        var taskData = await _dataProvider.GetTasksAsync();
        var projectsData = await _dataProvider.GetProjectsAsync();
        var userData = await _dataProvider.GetUsersAsync();
        var data = await taskData.GetTasksAsync();
        var user = await userData.GetUsersAsync();
        var projects = await projectsData.GetProjectsAsync();

        if (user.FirstOrDefault(u => u.Id == userId) != null)
        {
            return data.Where(t => t.PerformerId == userId).ToDictionary(
    ...
    public async Task<List<TaskDto>> GetCapitalTasksByUserIdAsync(int userId)
    {
        ...
        if (user.FirstOrDefault(u => u.Id == userId) != null)
        {
            return _mapper.Map<List<TaskDto>>(data.Where(d => d.PerformerId == userId & char.IsUpper(d.Name[0])));
        }
    ...
    public async Task<List<(int Id, string Name)>> GetProjectsByTeamSizeAsync(int teamSize)
    {
        ...
        var projectsData = await _dataProvider.GetProjectsAsync();
        var user = await usersData.GetUsersAsync();
        var projects = await projectsData.GetProjectsAsync();

        var list = new List<(int, string)>();

        var teams = user.GroupBy(t => t.TeamId).Where(i => i.Count() >= teamSize).Select(u => u.First().TeamId).ToList();

        if (teams != null)
        {
            foreach (var t in teams)
            {
                var project = projects.FirstOrDefault(p => p.TeamId == t);
    ...
    {
        var usersData = await _dataProvider.GetUsersAsync();
        var teamsData = await _dataProvider.GetTeamsAsync();

        var data = await usersData.GetUsersAsync();
        var teams = await teamsData.GetTeamsAsync();
    ...
    {
        var usersData = await _dataProvider.GetUsersAsync();
        var tasksData = await _dataProvider.GetTasksAsync();
        var users = await usersData.GetUsersAsync();
        var tasks = await tasksData.GetTasksAsync();
    ...
    public async Task<UserInfoDto> GetUserInfoAsync(int userId)
    {
        ...
        if (user != null)
        {
            var project = projects.Where(p => p.TeamId == user.TeamId).LastOrDefault();
            project = (project == null) ? null : project;

            var userLastTasks = (project == null) ? null : tasks.Where(t => t.ProjectId == project.Id).ToList();
    ...
    public async Task<PagedList<FullProjectDto>> GetSortedFilteredPageOfProjectsAsync(PageModel pageModel, FilterModel filterModel, SortingModel sortingModel)
    {
        ...
        var list = (sortingModel==null) ? data : SortList(data, sortingModel);

        var result = (filterModel==null & pageModel==null) ? list: ListFilter(list, filterModel, pageModel);
        return result;
